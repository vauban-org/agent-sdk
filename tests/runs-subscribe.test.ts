...
function makeSseEvent(name: string, data: unknown, id?: string): string {
...
describe("subscribeToRun", () => {
  ...
  it("forwards Last-Event-ID header on initial call when lastEventId is set", async () => {
    ...
    const chunks = [makeSseEvent("run_complete", { status: "succeeded" })];
  ...
  it("aborts stream immediately when signal is aborted before open", async () => {
    const mockFetch = vi
      .fn()
      .mockImplementation(() => new Promise<Response>((resolve) => setTimeout(resolve, 5_000)));
    vi.stubGlobal("fetch", mockFetch);
