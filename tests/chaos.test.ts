...
import { fullOutage, injectFailure, networkJitter } from "../src/testing/index.js";
...
describe("injectFailure", () => {
  ...
  it("fullOutage always fails with BrainUnavailable by default", async () => {
    ...
    await expect(dead.archiveKnowledge({ content: "x" })).rejects.toBeInstanceOf(BrainUnavailable);
  });
...
describe("chaos + circuit breaker integration", () => {
  it("fullOutage trips the circuit breaker and fast-fails", async () => {
    ...
    for (let i = 0; i < 3; i++) {
      await expect(protectedArchive({ content: "x" })).rejects.toBeInstanceOf(BrainUnavailable);
    }
    // 4th call short-circuits
    await expect(protectedArchive({ content: "x" })).rejects.toBeInstanceOf(CircuitOpenError);
  });

  it("bulkhead + network jitter keeps queue bounded", async () => {
    const slow = networkJitter(reliableBrain(), { minMs: 20, maxMs: 30 });
    const gated = bulkhead((entry: { content: string }) => slow.archiveKnowledge(entry), {
      name: "brain",
      maxConcurrent: 3,
      maxQueued: 5,
    });

    const promises = Array.from({ length: 8 }, () => gated({ content: "x" }).catch((err) => err));
