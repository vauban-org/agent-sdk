...
describe("CapabilityGate × AgentLoop (minimal-loop)", () => {
  it("denies a tool call when gate refuses; loop fires onToolDenied", async () => {
    ...
    const loop = new AgentLoop({
      ...
      capabilityGate: gate,
      onToolDenied: (e) => denied.push({ toolName: e.toolName, reason: e.reason }),
    });
    const result = await loop.run("hello");
    expect(result.stopReason).toBe("complete");
    expect(denied).toEqual([{ toolName: "fetch_github", reason: "tool_not_in_scope" }]);
  });

  it("ALLOW_ALL_GATE permits every call", async () => {
    const tools = makeRegistry();
    const provider = makeMockProvider([[{ name: "fetch_rss", args: {} }], []]);
    const loop = new AgentLoop({
      agentId: "tester",
      agentVersion: "test",
      systemPrompt: "sys",
