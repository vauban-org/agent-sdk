...
import { isReadOnlySql, runSqlQuery } from "../src/skills/run-sql-query.js";
...
describe("skill run_sql_query", () => {
  ...
  it("rejects write SQL with SqlReadOnlyViolation", async () => {
    ...
    await expect(runSqlQuery.execute({ sql: "DELETE FROM x" }, ctx)).rejects.toBeInstanceOf(
  ...
  it("uses dryRunMock when provided in replay mode", async () => {
    const mockResult = { rows: [{ x: 42 }], rowCount: 1 };
    const ctx = makeCtx({
      isReplay: true,
      dryRunMocks: { run_sql_query: () => mockResult },
    });
    const out = await runSqlQuery.execute({ sql: "SELECT x FROM t" }, ctx);
    expect(out.rowCount).toBe(1);
    expect((out.rows[0] as { x: number }).x).toBe(42);
  });

  it("isReadOnlySql allows WITH (CTE) SELECT", () => {
    expect(isReadOnlySql("WITH cte AS (SELECT 1) SELECT * FROM cte")).toBe(true);
  });

  it("isReadOnlySql blocks TRUNCATE", () => {
    expect(isReadOnlySql("TRUNCATE TABLE pipeline_runs")).toBe(false);
  });

  it("isReadOnlySql blocks CREATE TABLE inside comment-stripped SQL", () => {
    expect(isReadOnlySql("SELECT 1 /* CREATE TABLE x (a int) */")).toBe(true);
  });

  it("forwards params to db.query", async () => {
    const ctx = makeCtx({ isReplay: false, rows: [{ id: "p-1" }] });
    const out = await runSqlQuery.execute(
      { sql: "SELECT * FROM runs WHERE id = $1", params: ["p-1"] },
      ctx,
    );
    expect(out.rows[0]).toEqual({ id: "p-1" });
  });
});
