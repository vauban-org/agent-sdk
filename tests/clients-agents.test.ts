...
import { type AgentsClientOptions, createAgentsClient } from "../src/clients/agents.js";
...
describe("createAgentsClient", () => {
  ...
  describe("execute()", () => {
    ...
    it("uses backend runId when present in response", async () => {
      ...
      expect(result.runUrl).toBe("https://command.vauban.tech/runs/backend-run-42");
    ...
    it("throws on non-OK response", async () => {
      ...
      ).rejects.toThrow("AgentsClient.execute failed HTTP 500");
    });
  ...
  describe("listRegistry()", () => {
    ...
    it("throws on non-OK response", async () => {
      ...
      await expect(client.listRegistry()).rejects.toThrow(
        "AgentsClient.listRegistry failed HTTP 401",
      );
    });
  });
});
