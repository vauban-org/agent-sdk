...
describe("multiModalToAnthropicContent", () => {
  ...
  it("all fields combined → ordered blocks (text, image, audio, document)", () => {
    ...
    expect((blocks[2] as { type: string; text: string }).text).toMatch(/^data:audio\//);
  ...
  it("imageBase64 without text produces only an ImageBlock (no spurious TextBlock)", () => {
    const blocks = multiModalToAnthropicContent({ imageBase64: "data" });
    expect(blocks.filter((b) => b.type === "text")).toHaveLength(0);
    expect(blocks).toHaveLength(1);
  });

  it("imageMediaType image/gif is forwarded to the ImageBlock source", () => {
    const blocks = multiModalToAnthropicContent({
      imageBase64: "gifbytes",
      imageMediaType: "image/gif",
    });
    const img = blocks[0] as { type: string; source: { media_type: string } };
    expect(img.source.media_type).toBe("image/gif");
  });
});
