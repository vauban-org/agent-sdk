...
function makeAgentsMock(overrides: Partial<AgentsClient> = {}): AgentsClient {
  return {
...
function makePipelinesMock(overrides: Partial<PipelinesClient> = {}): PipelinesClient {
  return {
...
describe("triggerFromTelegram", () => {
  describe("/run command", () => {
    it("calls agents.execute with correct args and returns success reply", async () => {
      ...
      const result = await triggerFromTelegram(agents, pipelines, tgCtx, "/run", [
        "ARCHITECT",
        "analyse",
        "auth",
        "module",
    ...
    it("returns usage when only agentId provided (no description)", async () => {
      const agents = makeAgentsMock();
      const pipelines = makePipelinesMock();
      const result = await triggerFromTelegram(agents, pipelines, tgCtx, "/run", ["ARCHITECT"]);
      expect(agents.execute).not.toHaveBeenCalled();
      expect(result.replyText).toContain("Usage");
    });
  });
  ...
  describe("/pipeline command", () => {
    it("calls pipelines.run and returns queued reply", async () => {
      ...
      const result = await triggerFromTelegram(agents, pipelines, tgCtx, "/pipeline", [
        "daily-digest",
      ]);
  ...
  describe("/status command", () => {
    it("calls pipelines.status and returns status reply", async () => {
      ...
      const result = await triggerFromTelegram(agents, pipelines, tgCtx, "/status", [
        "run_123_abc",
      ]);
  ...
  describe("unknown command", () => {
    it("returns available commands list", async () => {
      const agents = makeAgentsMock();
      const pipelines = makePipelinesMock();
      const result = await triggerFromTelegram(agents, pipelines, tgCtx, "/help", []);
...
describe("triggerFromSlack", () => {
  describe("/run command", () => {
    it("calls agents.execute and returns success reply", async () => {
      ...
      const result = await triggerFromSlack(agents, pipelines, slackCtx, "/run", [
        "SYNERGY",
        "run",
        "competitive",
        "intel",
  ...
  describe("/pipeline command", () => {
    it("calls pipelines.run", async () => {
      ...
      const result = await triggerFromSlack(agents, pipelines, slackCtx, "/pipeline", [
        "cairo-sentinel",
      ]);
  ...
  describe("/status command", () => {
    it("calls pipelines.status", async () => {
      ...
      const result = await triggerFromSlack(agents, pipelines, slackCtx, "status", ["run_abc"]);
