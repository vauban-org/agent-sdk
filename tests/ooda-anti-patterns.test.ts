...
function baseCfg(): OODAAgentConfig<unknown, number, number, number, number, number> {
  return {
    agentId: "ap-test",
    intervalMs: 0,
    ...
    logger: noopLogger,
    phases: {
      observe: ro({ type: "observation", fn: async () => 1 }) as PhaseDef<void, number>,
      orient: ro({
        type: "retrieval",
...
describe("OODA anti-pattern #1 — sequential while+sleep, never setInterval", () => {
  it("agent.ts source contains no setInterval call", () => {
    const src = readFileSync(resolve(__dirname, "../src/orchestration/ooda/agent.ts"), "utf-8");
  ...
  it("non-readOnly observe rejected", () => {
    ...
    expect(() =>
      createOODAAgent({
        ...
        phases: {
          ...cfg.phases,
          observe: { type: "observation", fn: async () => 1 } as PhaseDef<void, number>,
        },
      } as OODAAgentConfig<unknown, number, number, number, number, number>),
    ).toThrow(/readOnly/);
  });
