...
function makeCtx(opts: { isReplay?: boolean } = {}): {
  ...
  const completeStep = vi.fn().mockResolvedValue({ leafHash: "0xabc" });
...
describe("withBrainContext", () => {
  ...
  beforeEach(() => {
    orientFn = vi.fn(
      async (input: OrientInputWithBrain<ObsInput>): Promise<OrientOutput> => ({
  ...
  it("missing fetchBrainContext + enabled → BrainSkillNotConfiguredError", async () => {
    ...
    await expect(wrapped({ query: "x" }, ctx)).rejects.toBeInstanceOf(BrainSkillNotConfiguredError);
