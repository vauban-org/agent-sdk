...
function makeOutcome(id: string, valueCents: number, isPending = false): Outcome {
  return {
    id,
    agent_id: "agent-test",
    agent_run_id: null,
...
describe("computeRoi", () => {
  ...
  it("handles all-positive values with no costs (no external map)", () => {
    const outcomes = [makeOutcome("a", 1000), makeOutcome("b", 500)];
    const result = computeRoi({ outcomes });
  ...
  it("uses external costsCentsByOutcomeId map when provided", () => {
    const outcomes = [makeOutcome("a", 3000), makeOutcome("b", 1000)];
