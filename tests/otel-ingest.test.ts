...
describe("createOtelClient", () => {
  ...
  it("sends Content-Type: application/json", async () => {
    vi.stubGlobal("fetch", vi.fn().mockResolvedValue(jsonResponse({ accepted: 0, warnings: [] })));
  ...
  it("sends X-Origin header when origin option is provided", async () => {
    vi.stubGlobal("fetch", vi.fn().mockResolvedValue(jsonResponse({ accepted: 1 })));
  ...
  it("throws on non-ok response with error message from body", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn().mockResolvedValue(jsonResponse({ error: "Rate limit exceeded" }, 429)),
    );
  ...
  it("serializes OTLP payload as JSON body", async () => {
    const mockFetch = vi.fn().mockResolvedValue(jsonResponse({ accepted: 1 }));
    vi.stubGlobal("fetch", mockFetch);
