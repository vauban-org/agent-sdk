...
- **From what source**: `git+https://github.com/vauban-org/command-center@<commit>`.
...
gh release download "sdk-v${VERSION}" \
  --repo vauban-org/command-center \
  --pattern "${PKG}.tgz" \
...
cosign verify-blob \
  ...
  --certificate-identity-regexp '.*vauban-org/command-center.*' \
...
- `build_config_digest`: commit SHA of `.github/workflows/sdk-publish.yml`
  at signing time.
- `source_repository`: `vauban-org/command-center`.
