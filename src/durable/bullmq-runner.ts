...
function jobOptsFromArchetype(archetype: QueueArchetype, cfg: BullMQRunnerConfig): JobsOptions {
...
export class BullMQRunner {
  private readonly cfg: Required<Pick<BullMQRunnerConfig, "redisDb" | "dlqName">> &
    BullMQRunnerConfig;
  ...
  constructor(config: BullMQRunnerConfig) {
    ...
    if (this.cfg.redisFactory) {
      this.connection = this.cfg.redisFactory(this.cfg.redisUrl, this.cfg.redisDb);
    } else {
      this.connection = new IORedis(this.cfg.redisUrl, this.sharedConnectionOpts);
    }
  }
  ...
  createQueue(name: string, archetype: QueueArchetype): Queue {
    if (!name || typeof name !== "string") {
      throw new Error("BullMQRunner.createQueue: name must be a non-empty string");
    }
  ...
  createWorker<T = unknown, R = unknown>(
    ...
    const policy = ARCHETYPE_POLICIES[archetype];
    const lockDuration = Math.max(30_000, Math.min(policy.timeoutMs, 30 * 60_000));
    ...
    logger.info({ queue: queueName, archetype, lockDuration }, "[bullmq] worker started");
    return worker;
  }
  ...
  dlqQueue(): Queue<DlqJobPayload> {
    if (!this._dlq) {
      this._dlq = this.createQueue(this.cfg.dlqName, "orchestration") as Queue<DlqJobPayload>;
    }
    return this._dlq;
  }
  ...
  async runFlowReliable<Parent = unknown>(
    ...
    const maxWaitMs = waitOpts.maxWaitMs ?? ARCHETYPE_POLICIES.orchestration.timeoutMs;
    ...
    while (Date.now() < deadline) {
      const parentJob = parentJobId ? await parentQueue.getJob(parentJobId) : null;
      if (!parentJob) {
        await sleep(pollMs);
        continue;
      }
      ...
      if (failedKeys.length > 0) {
        ...
        try {
          await parentJob.moveToFailed(new Error(cause), parentJob.token ?? "0", false);
        } catch (moveErr) {
          logger.debug({ err: moveErr, parentJobId }, "[bullmq] parent moveToFailed swallowed");
        }
        return {
          parentStatus: "failed",
  ...
  private attachDlqListener(queueName: string, archetype: QueueArchetype): void {
    ...
    const onFailed: QueueEventsListener["failed"] = async (args) => {
      try {
        ...
        const isTimeout =
          ...
          Boolean(job.processedOn && Date.now() - job.processedOn > policy.timeoutMs);
...
export function archetypePolicy(archetype: QueueArchetype): Readonly<ArchetypePolicy> {
