...
export const starknetBalance: Skill<StarknetBalanceInput, StarknetBalanceOutput> = {
  ...
  async execute(input, ctx: SkillContext): Promise<StarknetBalanceOutput> {
    ...
    return withSkillSpan("starknet_balance", async () => {
      ...
      if (!rpcUrl) {
        throw new SkillNotConfiguredError("starknet_balance", ["STARKNET_RPC_URL"]);
      }
      let starknet: typeof import("starknet");
      try {
        starknet = await import(/* @vite-ignore */ "starknet");
      } catch (_err) {
        throw new SkillNotConfiguredError("starknet_balance", ["starknet (peer dependency)"]);
      }
      const provider = new starknet.RpcProvider({ nodeUrl: rpcUrl });
      const tokenContract =
        input.contract ?? (input.token === "STRK" ? STRK : input.token === "ETH" ? ETH : undefined);
      if (!tokenContract) {
