...
async function loadOutcomesIndex(): Promise<Map<string, boolean>> {
  ...
  try {
    const [fs, yaml] = await Promise.all([import("node:fs/promises"), import("yaml")]);
    const raw = await fs.readFile(path, "utf-8");
    const parsed = yaml.parse(raw) as { outcomes?: OutcomesYamlEntry[] } | OutcomesYamlEntry[];
    const entries = Array.isArray(parsed) ? parsed : (parsed.outcomes ?? []);
