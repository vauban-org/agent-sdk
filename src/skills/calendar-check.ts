...
async function loadHolidays(): Promise<HolidayRecord[]> {
  ...
  try {
    ...
    const parsed = JSON.parse(raw) as HolidayRecord[] | { holidays?: HolidayRecord[] };
    CACHED_HOLIDAYS = Array.isArray(parsed) ? parsed : (parsed.holidays ?? []);
    return CACHED_HOLIDAYS;
  } catch (err) {
  ...
  async execute(input, ctx: SkillContext): Promise<CalendarCheckOutput> {
    ...
    return withSkillSpan("calendar_check", async () => {
      ...
      const match = holidays.find(
        (h) => h.date === input.date && (h.market === undefined || h.market.includes(input.market)),
      );
