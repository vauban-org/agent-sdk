...
async function apiFetch<T>(url: string, getToken: (() => Promise<string>) | undefined): Promise<T> {
  ...
  return fetchJson<T>(url, { headers }, { label: "RunsClient:" });
}
...
export interface RunsClient {
  ...
  subscribeToRun(
    runId: string,
    opts: Omit<SubscribeToRunOptions, "baseUrl" | "getToken">,
