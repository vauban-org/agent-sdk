...
export function verifyProofInclusion(leaf: string, proof: string[], root: string): boolean {
...
> = {
  ...
  outcome_ledger: (row) => {
    ...
    if (typeof v === "number" && !Number.isInteger(v)) {
      throw new Error("[proof] outcome_ledger.value_centimes must be integer centimes");
    }
    return {
      id: String(req(row, "id")),
      agent_id: String(req(row, "agent_id")),
...
export type { AssuranceGradingEngine, OtelSpan } from "./otel.js";
export { setAssuranceGradingEngine, toOtelSpan } from "./otel.js";
// Assurance attribute names (wire vocabulary). The grading engine itself is an
// internal workspace-only package (4.0.0 extraction) : the former grade/cone
// re-exports are gone from this public surface. toOtelSpan degrades explicitly
// when the engine is absent — see proof/otel.ts.
export {
