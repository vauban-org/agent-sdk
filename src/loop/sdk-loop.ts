...
/**
 * Default model when a caller does not pass one explicitly. Concrete ID
 * required by the Anthropic client — update alongside model releases (see
 * vauban-gouvernance model-selection.md model-ids-current marker).
 * last-verified 2026-07-08.
 */
const DEFAULT_SDK_MODEL = "claude-opus-4-8";
...
export interface SdkAgentLoopConfig {
  ...
  client: Anthropic;
  /** Model id (default: DEFAULT_SDK_MODEL, currently claude-opus-4-8). */
  model?: string;
...
class SdkAgentLoopImpl {
  ...
  constructor(config: SdkAgentLoopConfig) {
    this.config = {
      ...
      model: config.model ?? DEFAULT_SDK_MODEL,
  ...
  private async authoriseCall(
    ...
    if (capability === "mcp" && tool.mcpScopes) {
      if (!permitsMcpScopes(this.config.permissions, tool.mcpScopes)) return "deny";
    }
  ...
  private async awaitApproval(tool: AgentTool, args: unknown): Promise<boolean> {
...
function extractText(content: readonly Anthropic.Messages.ContentBlock[]): string {
  return content
