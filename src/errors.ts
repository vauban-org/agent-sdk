...
export class BrainRateLimit extends PortError {
  ...
  constructor(opts: { message?: string; retryAfterMs?: number; cause?: unknown } = {}) {
...
/**
 * Thrown by `triggerCycle()` when `opts.dryRun` is not explicitly set.
 *
 * Audit-ready agents must declare execution intent at every call site.
 * Passing `{dryRun: true}` for non-destructive runs, `{dryRun: false}` for
 * live execution, prevents accidental side-effect execution.
 *
 * @public
 */
export class MissingDryRunFlagError extends Error {
  constructor() {
    super(
      "triggerCycle() requires explicit dryRun flag (true|false). Audit-ready agents must declare execution intent.",
    );
    this.name = "MissingDryRunFlagError";
  }
}
