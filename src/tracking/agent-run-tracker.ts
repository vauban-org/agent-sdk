...
function assertNonNegativeInt(name: string, v: unknown): asserts v is number {
  if (!Number.isFinite(v) || !Number.isInteger(v) || (v as number) < 0) {
    throw new Error(`agent-run-tracker: ${name} must be a non-negative integer`);
  }
}

function assertNonNegativeCost(name: string, v: unknown): asserts v is number {
  if (!Number.isFinite(v) || (v as number) < 0) {
    throw new Error(`agent-run-tracker: ${name} must be a non-negative finite number`);
  }
}
  ...
  async recordStep(uuid: string, delta: AgentRunStepDelta): Promise<void> {
    ...
    await this.db.query(
      `UPDATE agent_run
       ...
       WHERE id = $1
         AND status = 'running'`,
      [uuid, delta.inputTokens, delta.outputTokens, delta.toolCalls ?? 0, costStr],
    );
  }

  async finish(uuid: string, result: AgentRunFinish): Promise<void> {
    assertUuid("uuid", uuid);
    const validStatuses: AgentRunFinalStatus[] = ["success", "failed", "timeout", "incoherent"];
    if (!validStatuses.includes(result.status)) {
      throw new Error(`agent-run-tracker: status must be one of ${validStatuses.join(", ")}`);
    }
    if (result.stopReason !== undefined && typeof result.stopReason !== "string") {
      throw new Error("agent-run-tracker: stopReason must be a string when provided");
    }
    if (result.errorMessage !== undefined && typeof result.errorMessage !== "string") {
      throw new Error("agent-run-tracker: errorMessage must be a string when provided");
    }
    ...
    await this.db.query(
      `UPDATE agent_run
       ...
       WHERE id = $1
         AND status = 'running'`,
      [uuid, result.status, result.stopReason ?? null, result.errorMessage ?? null],
    );
  }
}
